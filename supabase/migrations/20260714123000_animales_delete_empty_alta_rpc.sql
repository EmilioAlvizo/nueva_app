-- Migration: allow safe deletion of an alta with no linked animals.
--
-- Rollback guidance:
-- 1. Restore the previous `public.eliminar_alta_animales(uuid, boolean)` body.
...
begin
  ...
  select
    ...
    count(*),
    coalesce(bool_or(coalesce(activo, false) = false or baja_id is not null), false),
    coalesce(
